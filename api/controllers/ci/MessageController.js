const Messageci = require("../../models/ci/Message");
...
const sendMessage = async (req, res) => {
  ...
  try {
    ...
    const conversation = await Conversationci.findById(conversationId);
    if (!conversation) {
    ...
    // Create new message
    const newMessage = await Messageci.create({
      conversationId,
      senderId,
      message,
      isRead: false,
...
const getMessages = async (req, res) => {
  ...
  try {
    const messages = await Messageci.find({ conversationId });
...
const markMessageAsRead = async (req, res) => {
  ...
  try {
    await Messageci.findByIdAndUpdate(messageId, { isRead: true });
